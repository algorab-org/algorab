...
enum Expr:
  ...
  case ClassDef(internalName: Identifier, init: Expr, declarations: Chunk[(VariableId, Identifier)], exprType: Type)
  case Select(expr: Expr, name: Identifier, exprType: Type)
  ...
  def withType(tpe: Type): Expr = this match
    ...
    case ClassDef(internalName, init, definitions, _) => ClassDef(internalName, init, definitions, tpe)
