...
def typeExpr(expr: resolved.Expr): Typing[typed.Expr] = expr match
  ...
  case resolved.Expr.Less(left, right, span) =>
    typed.Expr.Less(assertNumeric(typeExpr(expr)), assertNumeric(typeExpr(expr)), typed.Type.Boolean, span)
  case resolved.Expr.LessEqual(left, right, span) =>
    typed.Expr.LessEqual(assertNumeric(typeExpr(expr)), assertNumeric(typeExpr(expr)), typed.Type.Boolean, span)
  case resolved.Expr.Greater(left, right, span) =>
    typed.Expr.Greater(assertNumeric(typeExpr(expr)), assertNumeric(typeExpr(expr)), typed.Type.Boolean, span)
  case resolved.Expr.GreaterEqual(left, right, span) =>
    typed.Expr.GreaterEqual(assertNumeric(typeExpr(expr)), assertNumeric(typeExpr(expr)), typed.Type.Boolean, span)
