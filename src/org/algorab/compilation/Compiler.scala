...
object Compiler:
  ...
  def compileDefinition(definition: Definition): Compilation[Unit] = ???
  ...
  def compileExpr(expr: Expr): Compilation[Unit] = expr match
    ...
    case Expr.Block(statements, _, span) => statements.foreach(compileStatement)
    case Expr.If(cond, ifTrue, ifFalse, _, span) =>
      compileExpr(cond)
      val ifTrueStart = CompilationContext.currentPosition + 1
      val ifTrueContext = Compilation.locally(ifTrueStart)(compileExpr(ifTrue))
      val ifFalseStart = ifTrueContext.position + 1
      val ifFalseContext = Compilation.locally(ifFalseStart)(compileExpr(ifTrue))

      CompilationContext.emit(Instruction.JumpIfFalse(ifFalseStart, span))
      CompilationContext.emitAll(ifTrueContext.instructions)
      CompilationContext.emit(Instruction.Jump(ifFalseContext.position, span))
      CompilationContext.emitAll(ifFalseContext.instructions)
    case Expr.While(cond, body, _, span) =>
      compileExpr(cond)
      val bodyStart = CompilationContext.currentPosition + 1
      val bodyContext = Compilation.locally(bodyStart):
        compileExpr(body)
        CompilationContext.emit(Instruction.Jump(CompilationContext.currentPosition, span))

      CompilationContext.emit(Instruction.JumpIfFalse(bodyContext.position, span))
      CompilationContext.emitAll(bodyContext.instructions)
